fun main(args: Array<String>) {
    ...
    val numbersMap = mapOf("key1" to 1, "key2" to 2, "key3" to 3, "key 4" to 1)
    ...
    val anotherMap = mapOf("key1" to 1, "key2" to 2, "key 4" to 1, "key3" to 3 )
    println("The maps are equal: ${numbersMap == anotherMap}")

    //MutableMap is a map with write operations, for example
    //you can add a new key-value pair or update the value associated with the given key
    val  mutableMap = mutableMapOf("one" to 1,"bar" to 2)
    mutableMap.put("three", 3)
    mutableMap["one"] = 11
    println(mutableMap)

    val readWriteMap = hashMapOf("foo" to 1, "bar" to 2 )
    println(readWriteMap["foo"]) //print 1
}